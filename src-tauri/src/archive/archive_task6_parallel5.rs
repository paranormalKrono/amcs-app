...
//         // Multithreading
//         let (tx012, rx012) = {
//             let data = mpsc::channel::<Vec<Vec<f64>>>();
//             (data.0, Arc::new(Mutex::new(data.1)))
//         };
//         let (tx10, rx01) = mpsc::channel::<Vec<Vec<f64>>>();
//
//         let (tx12, rx21) = mpsc::channel::<Vec<Vec<f64>>>();
//         let (tx21, rx12) = mpsc::channel::<Vec<Vec<f64>>>();
//
//         // Functions values
...
//                         let rx01 = rx012.clone();
//                         thread::spawn(move || {
//                             parallel5_run_method_first(rx01, tx10, tx12, rx12);
//                         });
//                         thread::spawn(move || {
//                             parallel5_run_method_secondary(rx012, rx21, tx21);
//                         });
//                     }
...
//                                     .unwrap();
//                                 tx012
//                                     .send(vec![av0.clone(), av01.clone(), av02.clone(), avs0])
//                                     .unwrap();
//                                 avs0 = rx01.recv().unwrap().remove(0);
//                             }
//                             OptimizationType::Multithreading => {}
//                         }
//
...
//                                     .unwrap();
//                                 tx012
//                                     .send(vec![av1.clone(), av11.clone(), av12.clone(), avs1])
//                                     .unwrap();
//                                 avs1 = rx01.recv().unwrap().remove(0);
//                             }
//                             OptimizationType::Multithreading => {}
//                         }
//
...
// }
//
// fn parallel5_run_method_secondary(
//     rx20: Arc<Mutex<Receiver<Vec<Vec<f64>>>>>,
//     rx21: Receiver<Vec<Vec<f64>>>,
//     tx21: Sender<Vec<Vec<f64>>>,
// ) {
//     loop {
//         let (l, mut d, r, mut b) = {
//             let rec = rx20.lock().unwrap().recv();
//             //println!("B");
...
//         }
//
//         tx21.send(vec![b[..n2].to_vec()]).unwrap();
//         //println!("B3");
//     }
// }
//
// fn parallel5_run_method_first(
//     rx10: Arc<Mutex<Receiver<Vec<Vec<f64>>>>>,
//     tx10: Sender<Vec<Vec<f64>>>,
//     tx12: Sender<Vec<Vec<f64>>>,
//     rx12: Receiver<Vec<Vec<f64>>>,
// ) {
//     loop {
//         let (l, mut d, r, mut b) = {
//             let rec = rx10.lock().unwrap().recv();
//             //println!("A");
//             let mut val = match rec {
//                 Ok(value) => value,
//                 Err(_) => {
...
//         }
//
//         tx12.send(vec![vec![d[n2], b[n2]]]).unwrap();
//         //println!("A1");
//         b[n2] = rx12.recv().unwrap()[0][0];
//         d[n2] = 1_f64;
//
//         for i in n2..(b.len() - 1) {
...
//         }
//
//         b[..n2].clone_from_slice(&rx12.recv().unwrap()[0]);
//         //println!("A2");
//
//         tx10.send(vec![b]).unwrap();
//         //println!("A3");
//     }
// }
//
